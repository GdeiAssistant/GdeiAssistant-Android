...
import cn.gdeiassistant.Activity.LoginActivity;
import cn.gdeiassistant.Activity.MainActivity;
import cn.gdeiassistant.Application.GdeiAssistantApplication;
import cn.gdeiassistant.Constant.ActivityRequestCodeConstant;
import cn.gdeiassistant.Constant.MainTagConstant;
import cn.gdeiassistant.Constant.PermissionRequestCodeConstant;
import cn.gdeiassistant.Constant.RequestConstant;
import cn.gdeiassistant.Model.BitmapFileModel;
import cn.gdeiassistant.Model.MainModel;
import cn.gdeiassistant.Pojo.Entity.Access;
import cn.gdeiassistant.Pojo.Entity.Profile;
import cn.gdeiassistant.R;
import cn.gdeiassistant.Service.UpgradeService;
import cn.gdeiassistant.Tools.TokenUtils;
...
public class MainPresenter {
    ...
    private void Init() {
        ...
        if (receiver == null) {
            receiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    if (intent.getAction() != null) {
                        if (intent.getAction().equals("cn.gdeiassistant.CHECK_UPGRADE")) {
            ...
            //注册广播监听
            mainActivity.registerReceiver(receiver, new IntentFilter("cn.gdeiassistant.CHECK_UPGRADE"));
        }
        //加载用户权限列表信息
        GetUserAccess();
    }
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Request request = new Request.Builder().post(requestBody).url(mainActivity.getApplicationContext()
                                .getString(R.string.resource_domain) + "rest/token/expire").build();
    ...
    public void GetPhotoFromCamera() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri uri = FileProvider.getUriForFile(mainActivity.getApplicationContext(), "cn.gdeiassistant.fileprovider", file);
    ...
    public void StartCamera(Context context) {
        ...
        if (tempFile.exists() && tempFile.canRead()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Uri uri = FileProvider.getUriForFile(context, "cn.gdeiassistant.fileprovider", tempFile);
                StartPhotoZoom(uri);
            } else {
